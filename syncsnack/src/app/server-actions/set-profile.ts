...
export async function joinGroup(userId: string | null, groupName: string, groupPassword: string) {
  try {
    const response = await fetch(`${process.env.NEXT_PUBLIC_BACKEND_URL}/api/groups/join`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ userId, name: groupName, password: groupPassword }),
    });
...
export async function setupProfile(userId: string | null, formData: any) {
  ...
  if (formData.groupChoice === 'create') {
    ...
    const joinResult = await joinGroup(userId, formData.groupName, formData.groupPassword);
    if (joinResult.error) {
